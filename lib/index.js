...
var parseNamespacesFromEnvironment = function parseNamespacesFromEnvironment() {
	var namespaces = process.env.DEBUG || defaultOptions.namespaces;
...
var Logger = function(namespace) {
	...
	var log = function(level) {
		...
		switch(level) {
			case "error":
				error.apply(this, newArguments);
				break;
			case "warn":
				warn.apply(this, newArguments);
				break;
			case "info":
				info.apply(this, newArguments);
				break;
			case "debug":
				debug.apply(this, newArguments);
				break;
		}
	};
	var error = function(){
